    ...
    return (
        <div ref={ref} className="flex flex-col justify-center items-center space-y-2.5 md:space-y-5 overflow-hidden">
            <motion.h1 variants={{
                ...
                <span className="text-red-shade-1">{HighlightText}</span> {Text}
            </motion.h1>
            <motion.p variants={{
                hidden: CaptionAnimation,
                visible: VisibleCaptionAnimation
            }} initial="hidden" animate={mainControls} transition={{ duration: 0.5, delay: 0.5 }} className=" font-extralight text-white-shade-50 text-xs md:text-base text-center mx-2 w-full md:max-w-[1200px]">
                {Caption}
            </motion.p>
        </div>
    );
